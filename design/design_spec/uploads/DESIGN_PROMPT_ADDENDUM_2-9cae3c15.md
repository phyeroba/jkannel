...
9. **Withdraw flow** — from the balance card, exactly these fields:
   **amount** (with a "withdraw all available" shortcut), **phone number**
   to pay out to (mobile money, pre-filled from profile, editable with
   confirmation), **reason for the withdrawal** (required free text), then
   **Submit** — which dispatches to the payout gateway.
   The amount validation must respect the **reserve fund**: every owner has
   a minimum balance that cannot be withdrawn, so show the math plainly —
   "Balance UGX 250,000 · Reserve UGX 10,000 · **Withdrawable UGX 240,000**"
   — and cap the input (and the "withdraw all" shortcut) at withdrawable,
   never at balance. Attempting more shows why, not just an error.
   Review step, then a submitted state: "Withdrawal requested — usually
   paid within X". Statuses the design must carry: **pending → approved →
   paid** or **rejected** (with reason).
