...
describe('application shell', () => {
  it('exposes navigation and skip-link landmarks with accessible labels', async () => {
    ...
    // Services and Nodes to System; 44 before System's Performance, Data
    // Integrity and Background Jobs, Traffic's Delivery Retries, and
    // Messaging's Recipient Policy and Scheduled Sends.
    expect(wrapper.get('aside[aria-label="Primary navigation"] nav').findAll('a')).toHaveLength(50);
    expect(wrapper.findAll('.nav-items .nav-icon svg')).toHaveLength(50);
