...
## 5. Further suggestions (2026-07-10, cycle 3)

With the console modules, analytics substrate, and report catalog now in place, these are the highest-value next additions. Each builds on infrastructure that already exists.

### Reports and analytics (the report catalog is now a framework — wire more kinds into it)
1. **Per-SMSC success/failure & availability report** — extend the daily snapshot to record delivered/failed/rejected and bind uptime per SMSC (the SMSC spec §6 fields). The catalog already lists "Success rate by SMSC" as available; deepen it with failure-code breakdowns.
2. **Per-route performance & failover report** — route success rate, latency, and failover-event counts; needs route-level attribution on messages (today routes are attributed via target SMSC).
3. **Hourly traffic heatmap** — a day×hour heatmap of message volume (Dashboard spec calls for heat maps). The MiniChart component can be extended to render it.
4. **SLA / delivery-latency report** — time-to-DLR distribution (p50/p95/p99) per SMSC and route, with an SLA threshold line.
5. **Financial / cost & margin report** — add per-route `cost` and per-customer `rate` fields, then report spend, revenue, and margin. Unlocks the "Financial/Vendor" catalog category currently marked *planned*.
6. **Scheduled report exports** — let operators schedule a report (daily/weekly) to be emailed/webhooked as a PDF; the report-jobs scheduler and export service already exist, so this is mostly a schedule table + delivery wiring.
7. **Saved report views** — persist a user's chosen filters/range/columns as a named report they can re-run (small once grids are in place).
8. **Executive one-page PDF** — a branded executive summary (KPIs + top charts) exportable as a single PDF for stakeholders.

### Operations & messaging
9. **Message replay / clone / requeue** — the Message Explorer spec calls for replay; add an audited "resend" that re-enqueues a `sent_sms` row through `send_sms`.
10. **Bulk send / campaigns** — a bounded bulk-submit job (CSV upload → validated → enqueued) using the existing job ledger for progress.
11. **Live SMSC status polling** — a lightweight poller that refreshes SMSC bind state on an interval and pushes changes to the dashboard (today status updates on action).
12. **SMPP synthetic canary** — a periodic canary submit + DLR round-trip check per route for true end-to-end health (distinct from process-up health).
13. **Config drift detection** — hash the live engine config and compare to the active configuration version; raise an alert on drift.

### Notifications & alerting
14. **Notification preferences per user** — per-category mute and preferred channel (in-app/email/webhook), building on the notification centre and channel model.
15. **Alert escalation chains & maintenance windows** — the alerts spec (§10–25) defines escalation, dedup correlation, and maintenance windows; the delivery layer and channels already exist.
16. **Slack/Telegram channel adapters** — concrete channel types behind the existing notification-delivery boundary (the example Slack plugin is a scaffold).

### AI (extend the Copilot beyond read-only Q&A, keeping human approval)
17. **AI config-review** — send the redacted config diff to the model before deployment for a risk annotation on the approval screen (human approval still mandatory).
18. **AI alert triage** — cluster related alerts and draft an incident summary with a ranked probable cause and *suggested* (never executed) remediation.
19. **Predictive volume/anomaly forecasting** — move anomaly detection from simple statistics to a seasonal/ML baseline for earlier, lower-false-positive detection.
20. **Natural-language report builder** — "weekly Nigeria traffic by SMSC as PDF" → a structured report definition executed by the reporting engine (the analytics endpoints are the substrate).

### Security & reliability
21. **MFA (TOTP)** with per-role enforcement; **refresh-token family revocation** on replay detection.
22. **Scheduled encrypted backups + one-click restore UI** — turn the backup catalog into real scheduled `pg_dump` artifacts (the DR smoke script already does this) with retention and a guarded restore workflow.
23. **Historical partitioning/archive jobs** for messages, DLRs, audit, and metrics (retention settings now exist to drive them).
24. **API-gateway rate-limit enforcement** — enforce the per-client `rate_limit_per_min` at the edge (the field and client registry exist).

Recommended near-term order: (6) scheduled report exports and (1–4) the additional report kinds — they directly extend what was just built and give operators immediately useful, different reports; then (22) real backups and (23) archival before production traffic; then (15) alert escalation and (17) AI config-review.
